...
namespace football.history.api.Tests.Builders.LeagueTable.Sorter
{
    ...
    public class PointsGoalDiffGoalsForComparerTests
    {
        private static IEnumerable Compare_variations()
        {
            ...
            yield return new TestCaseData(
                new LeagueTableRowDto {Team = "Norwich City", Points     = 1, GoalDifference = 0, GoalsFor = 0, GoalAverage = 1},
                new LeagueTableRowDto {Team = "Newcastle United", Points = 0, GoalDifference = 1, GoalsFor = 1, GoalAverage = 0},
                1).SetName("when points are different");

            yield return new TestCaseData(
                new LeagueTableRowDto {Team = "Norwich City", Points     = 0, GoalDifference = 1, GoalsFor = 0, GoalAverage = 1},
                new LeagueTableRowDto {Team = "Newcastle United", Points = 0, GoalDifference = 0, GoalsFor = 1, GoalAverage = 0},
                1).SetName("when points are equal but goal diff is different");

            yield return new TestCaseData(
                new LeagueTableRowDto {Team = "Norwich City", Points     = 0, GoalDifference = 0, GoalsFor = 1, GoalAverage = 1},
                new LeagueTableRowDto {Team = "Newcastle United", Points = 0, GoalDifference = 0, GoalsFor = 0, GoalAverage = 0},
                1).SetName("when points and goal diff are equal but goals for is different");
            
            yield return new TestCaseData(
                new LeagueTableRowDto {Team = "Norwich City", Points     = 0, GoalDifference = 0, GoalsFor = 0, GoalAverage = 1},
                new LeagueTableRowDto {Team = "Newcastle United", Points = 0, GoalDifference = 0, GoalsFor = 0, GoalAverage = 0},
                -1).SetName("when points and goal diff and goals for are equal");
            
            yield return new TestCaseData(
                new LeagueTableRowDto {Team = "Norwich City", Points = 0, GoalDifference = 0, GoalsFor = 0, GoalAverage = 1},
                new LeagueTableRowDto {Team = "Norwich City", Points = 0, GoalDifference = 0, GoalsFor = 0, GoalAverage = 0},
